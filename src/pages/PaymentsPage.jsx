...
function PaymentsPage({ appData, setAppData, notify }) {
  ...
  const filteredPayments = useMemo(() => {
    const next = [...payments].filter((payment) => {
      const matchesSearch = !search || [payment.user, payment.property, payment.id].join(' ').toLowerCase().includes(search.toLowerCase());
  ...
  const handleSave = (event) => {
    ...
    setAppData((prev) => ({
      ...
      dues: (prev.dues || []).map((due) =>
        due.user === payload.user && due.property === payload.property && payload.status === 'Paid'
          ? { ...due, status: 'Paid', paymentStatus: 'Paid', updatedAt: new Date().toISOString() }
          : due,
      ),
    }));
      ...
      <section className="panel-card toolbar-card">
        <div className="toolbar-row">
          <div className="toolbar-search">
            <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by user or property" />
          </div>
          <div className="toolbar-group">
            ...
            <button type="button" className="primary-button" onClick={() => setModalOpen(true)}>Record Payment</button>
          </div>
        </div>
      </section>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <button type="submit" className="primary-button" form="payment-form">Save</button>
          </>
        }
      >
        <form id="payment-form" className="property-form" onSubmit={handleSave}>
          <div className="form-grid two-col">
            <label>
              User
              <input value={formData.user} onChange={(event) => setFormData((prev) => ({ ...prev, user: event.target.value }))} />
              {errors.user && <span className="field-error">{errors.user}</span>}
            </label>
            <label>
              Property
              <input value={formData.property} onChange={(event) => setFormData((prev) => ({ ...prev, property: event.target.value }))} />
          ...
          <div className="form-grid two-col">
            ...
            <label>
              Method
              <select value={formData.method} onChange={(event) => setFormData((prev) => ({ ...prev, method: event.target.value }))}>
          ...
          <label>
            Note
            <textarea rows="3" value={formData.note} onChange={(event) => setFormData((prev) => ({ ...prev, note: event.target.value }))} />
          </label>
        </form>
      </Modal>
    </div>
